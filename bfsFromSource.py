...
pygame.display.set_caption("BFS From Source Only!")
...
def bfs_from_source(draw, grid, start, end):
    ...
    while queue:
        ...
        if current == end:
            ...
            print('Shortest Distance = {}'.format(cost[end]))
            end.make_end()
            return True
        for neighbor in current.neighbors:
            if cost[neighbor] == float("inf") or cost[neighbor] > cost[current] + 1:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    bfs_from_source(lambda: draw(win, grid, ROWS, width), grid, start, end)
